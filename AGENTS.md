...
- Speech input normalizes Java whitespace, Unicode separator characters, and
  control characters; format-only or combining-mark-only input is rejected
  while visible decomposed text and emoji sequences are preserved. Input must
  be non-empty and is capped at 200 characters before platform TTS dispatch.
